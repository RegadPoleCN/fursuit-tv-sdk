...
| 参数 | 类型 | 默认值 | 必填 | 说明 |
|------|------|--------|------|------|
| `clientId` | `String?` | `null` | 条件 | 应用 ID（即 VDS 文档中的 `appId`），格式为 `vap_xxxx` |
...
| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `logLevel` | `SdkLogLevel` | `SdkLogLevel.INFO` | HTTP 日志级别 |

**日志级别**:
```kotlin
SdkLogLevel.OFF      // 关闭所有日志
SdkLogLevel.ERROR    // 仅错误
SdkLogLevel.WARNING  // 警告及以上
SdkLogLevel.INFO     // 信息及以上（默认）
SdkLogLevel.DEBUG    // 调试及以上
SdkLogLevel.ALL      // 所有
```
...
| 3 | `clientId` | `String?` | `null` | 应用 ID（即 VDS 文档中的 `appId`，格式 `vap_xxxxxxxx`，32位十六进制） |
...
| 8 | `logLevel` | `SdkLogLevel` | `INFO` | HTTP 客户端日志级别（OFF/ERROR/WARNING/INFO/DEBUG/ALL） |
