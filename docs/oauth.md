...
本文档详细说明 Fursuit.TV SDK 的 OAuth 2.0 实现，包括完整的授权流程和使用方法。

**注意**：本文档仅说明 OAuth 特有的内容。认证方式和配置选项请查看 [认证与配置](authentication.md)。
...
1. 在 VDS 开放平台创建应用，获取 `clientId`（格式：vap_xxxx）和 `clientSecret`
...
// 第三步：启动 OAuth 流程（自动使用 SDK 配置中的 clientId 和 clientSecret）
