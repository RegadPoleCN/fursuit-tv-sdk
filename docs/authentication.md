...
| **认证头** | `X-Api-Key`（推荐）/ `Authorization: Bearer` | `Authorization: Bearer`（仅 getUserInfo） |
...
签名交换是 SDK 的主要认证方式，通过应用凭证（clientId + clientSecret）向 VDS 服务器请求平台签名：
...
>
> 如果未完成前置条件就调用 initOAuth()，将抛出 IllegalStateException：
> ```
> IllegalStateException: Platform access token is required for OAuth flow. Call exchangeToken first.
> ```
...
// 输出: https://open-global.vdsentnet.com/api/account/sso/authorize?...
```
...
**启用方式**：在 OAuthConfig 中设置 `enablePkce = true`
...
> **推荐**: 生产环境务必启用 PKCE（`enablePkce = true`）

### 自动化流程：initOAuth()

为了简化 OAuth 流程，SDK 提供了 `initOAuth()` 方法，一站式处理整个流程：
...
// Step 3: 启动 OAuth 流程（自动处理回调服务器、浏览器、令牌交换）
...
**initOAuth() 自动完成的操作**：
1. 启动本地 HTTP 服务器监听回调（默认 localhost:8080）
2. 打开系统默认浏览器访问授权 URL
3. 等待用户授权并接收回调
...
| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| callbackHost | String | `"localhost"` | 回调服务器主机名 |
| callbackPort | Int | `8080` | 回调服务器端口（建议 >1024） |
| callbackPath | String | `"/callback"` | 回调路径 |
| stateTimeoutMinutes | Int | `5` | 授权超时时间（分钟） |
| enablePkce | Boolean | `false` | 是否启用 PKCE（推荐 true） |

### 手动流程 vs 自动流程

| 特性 | 手动流程 | 自动流程 (initOAuth) |
|------|---------|---------------------|
| 适用场景 | Web 服务器、自定义 UI | 桌面/移动应用、快速原型 |
| 回调处理 | 自己实现 HTTP 端点 | SDK 自动启动临时服务器 |
...
### 双认证头机制（getUserInfo 专用）

当调用 `getUserInfo()` 方法时，SDK 会同时发送两个认证头：

```
GET /api/account/sso/userinfo HTTP/1.1
Authorization: Bearer <platformAccessToken>     ← 验证应用身份
X-OAuth-Access-Token: <oauthAccessToken>       ← 标识用户身份
```
...
val sdk = fursuitTvSdk {
    ...
    maxRetries = 3               // 最大重试3次
    logLevel = LogLevel.BODY     // 生产环境可改为 NONE 或 ERROR
}
```
...
**症状**: 抛出 `IllegalStateException: Platform access token is required...`

**原因**: 直接调用 `initOAuth()` 但之前没有完成签名交换

**解决方案**:
```kotlin
// ❌ 错误：直接创建空 SDK 并尝试 OAuth
val sdk = FursuitTvSdk.createEmpty()
sdk.auth.initOAuth(config)  // 💥 异常！

// ✅ 正确：先通过 DSL 初始化（自动完成签名交换）
val sdk = fursuitTvSdk { clientId = "..."; clientSecret = "..." }
sdk.auth.initOAuth(config)  // ✅ 正常工作
```
...
**原因**: oauthAccessToken 只能用于 `getUserInfo()`，不能用于其他 API
...
val profile = sdk.user.getUserProfile("username")  // 使用 apiKey
